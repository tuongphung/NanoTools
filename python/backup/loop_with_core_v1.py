...
def get_bin_num(ccpt, eta):
    i = bisect(x_bins, ccpt)
    j = bisect(y_bins, abs(eta))
    return (i, j)

bin_content = {(i,j): [] for i in range(1, n_xbins) for j in range (1, n_ybins)}
gen_ids = []
